...
- **Derived, never stored:**
  ...
  - `done-merged`, ✅ on the local mainline or on `<remote>/<mainline>` unless the other of the two has a `Reopens: <ID>` commit that ref lacks (*T064*), or a merge `autopilot merged`
  ...
  - `discarded`, ❌ in the checkout, or on the local mainline or `<remote>/<mainline>` under the same reopen rule (*T064*), so a
