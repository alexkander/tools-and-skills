...
- **Derived, never stored:**
  ...
  - `discarded-branch`, ❌ at the task branch tip but closed on neither mainline ref (*T062*): the
    lane has ended, it uses no lane and frees its place, and it is not in the hand-off queue;
  - `discarded`, ❌ in the checkout, on the local mainline or on `<remote>/<mainline>`, so a
    discarded run task stays visible.
