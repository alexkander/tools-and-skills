...
Status: **v1 released as `taskrail-v0.1.0`.** See CHANGELOG.md.
...
```toml
version = "taskrail-v0.1.0"          # CLI version the repository is pinned to
...
```bash
uv tool install taskrail --from "git+https://github.com/alexkander/tools-and-skills.git@taskrail-v0.1.0#subdirectory=tools/taskrail"
taskrail init --integration claude
```

Tags are prefixed per tool (`taskrail-vX.Y.Z`) so other tools in this repository can version
independently.
...
- **Derived, never stored:**
  - `pending`;
  - `running`, a live claim, or — between `taskrail done` and its commit — the row `✅` in the
