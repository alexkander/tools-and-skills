...
def test_upgrade_pins_the_version_and_restores_skills(empty_repo, capsys):
    ...
    config.write_text(config.read_text().replace(install.release_tag(), "taskrail-v0.0.1"))
...
def test_self_upgrade_dry_run(capsys, monkeypatch):
    code = main(["self", "upgrade", "--tag", "taskrail-v0.1.0", "--dry-run"])
    out = capsys.readouterr().out
    assert code == 0
    assert "uv tool install --force taskrail --from git+https://github.com/alexkander/tools-and-skills.git@taskrail-v0.1.0#subdirectory=tools/taskrail" in out


def test_release_tag_drops_development_suffixes():
    assert install.release_tag("0.1.0.dev0") == "taskrail-v0.1.0"
...
def test_wrapper_with_a_local_pin_runs_the_source_in_this_checkout(empty_repo, capsys, monkeypatch):
    ...
    init(empty_repo, capsys=capsys)
    config = empty_repo / ".taskrail/config.toml"
    source = Path(install.__file__).resolve().parents[2]  # tools/taskrail
...
def test_upgrade_keeps_a_local_pin(empty_repo, capsys):
    init(empty_repo, capsys=capsys)
    config = empty_repo / ".taskrail/config.toml"
    config.write_text(config.read_text().replace(f'"{install.release_tag()}"', '"local:tools/taskrail"'))
    assert run(empty_repo, "upgrade", capsys=capsys)[0] == 0
    assert 'version = "local:tools/taskrail"' in config.read_text()
