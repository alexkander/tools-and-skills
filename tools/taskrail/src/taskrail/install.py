...
SOURCE_URL = "https://github.com/alexkander/tools-and-skills.git"
...
def release_tag(version: str = __version__) -> str:
    ...
    return f"taskrail-v{base.group(0) if base else version}"
...
if [ -n "$installed" ] && [ "$installed" != "$0" ]; then
  ...
  if [ -z "$pin" ] || [ "taskrail-v$have" = "$pin" ]; then
    exec taskrail "$@"
  fi
fi
if [ -z "$pin" ]; then
...
exec uvx --quiet --from "git+${{TASKRAIL_SOURCE:-{SOURCE_URL}}}@$pin#subdirectory=tools/taskrail" taskrail "$@"
'''
...
def latest_tag(source: str = SOURCE_URL) -> str | None:
    output = gitutil.run(Path.cwd(), "ls-remote", "--tags", "--refs", source, "taskrail-v*").stdout
    tags = [line.rsplit("refs/tags/", 1)[1] for line in output.splitlines() if "refs/tags/" in line]

    def key(tag: str):
        return tuple(int(part) if part.isdigit() else 0 for part in re.split(r"[.-]", tag.removeprefix("taskrail-v")))

    return max(tags, key=key) if tags else None


def self_upgrade_command(tag: str, source: str = SOURCE_URL) -> list[str]:
    return ["uv", "tool", "install", "--force", "taskrail", "--from", f"git+{source}@{tag}#subdirectory=tools/taskrail"]
...
def self_upgrade(tag: str | None, dry_run: bool) -> tuple[list[str], int]:
    ...
    if tag is None:
        raise LookupError(f"no taskrail-v* release tag found at {source}")
