...
- `.taskrail/config.toml` — add an `[autopilot]` table:
  ...
  - `governing = ["CLAUDE.md", "tools/taskrail/DESIGN.md"]`, the documents the orchestrator reads
...
- The `test` check (`uv run --directory tools/taskrail pytest -q`) passes; `lint` is not configured.
