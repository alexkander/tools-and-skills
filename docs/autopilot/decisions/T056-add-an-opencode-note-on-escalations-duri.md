...
`c7cf7f0`), the `taskrail-autopilot` section of `tools/taskrail/src/taskrail/integrations/opencode.md`
...
| # | Question | Options | Decision | Reason |
|---|---|---|---|---|
| 1 | Conflict in `docs/chores/README.md`, `docs/autopilot/decisions/README.md` and `tools/taskrail/CHANGELOG.md` | keep both · stop | **keep both** | Class 2. The CHANGELOG bullet T056 moved to the end of *Unreleased* is there once. |
| 2 | Conflict in `TODO.md` | unite rows by ID · stop | **unite by ID** | Class 1. |
| 3 | Conflict in `tools/taskrail/DESIGN.md` §8 integrations table | stop · keep each task's row | **`main`'s `claude` row (T052) with T056's `opencode` row** | Each task changed a different row of the same table. Decided under the human's delegation of DESIGN.md decisions. |
