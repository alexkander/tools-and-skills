...
- `tools/taskrail/src/taskrail/autopilot/notify.py` (new): message composition, running the
  command, the result.
- `tools/taskrail/src/taskrail/autopilot/escalation.py` (new): governing-path matching and the
  per-task flags, so `status.py` gains only the `gate` field and one call in its own function.
- `tools/taskrail/src/taskrail/autopilot/status.py`: `gate` in the lane details and one small
