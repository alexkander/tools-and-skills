...
- `tools/taskrail/src/taskrail/query.py` — `base_dict` resolves the remote and adds `remote` and
  `remote_source`.
- `tools/taskrail/src/taskrail/cli.py` — `_open_workspace` and `cmd_review` use the resolved
  remote for base, fetch, push and link; the text output of `show` if it prints the base.
- `tools/taskrail/src/taskrail/install.py` — the comment on `[review].remote` in the default
  config: it is the fallback.
- `tools/taskrail/src/taskrail/skills/taskrail/SKILL.md` — step 3 fetches the mainline's remote
...
- `tools/taskrail/tests/test_review.py`, `tests/test_workspace.py` — tests with two local bare
  repositories as remotes; no network.
- `tools/taskrail/DESIGN.md`, `README.md`, `CHANGELOG.md` (one line under `## Unreleased`).
