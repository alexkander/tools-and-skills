...
- `tools/taskrail/src/taskrail/checks.py` — a new `chosen_resources(project, task_id, pairs)`
  ...
  updated.
- `tools/taskrail/src/taskrail/cli.py` — `cmd_checks` passes `args.resource`; the `checks` parser
  registration gains `--resource` (`action="append"`, `metavar="NAME=VALUE"`). No other command.
- `tools/taskrail/tests/test_checks.py` — new tests for criteria 1–5.
- `tools/taskrail/tests/test_autopilot_skill.py` — `test_refill_runs_once_a_close_is_reviewed_not_at_hand_off`
  (the assertion on *Close and hand off*) and one assertion on gate-review's *Rebase* section, for
  criterion 6.
- `tools/taskrail/src/taskrail/skills/taskrail-autopilot/SKILL.md` — *Close and hand off* step 1,
  its last sentence only (D5).
- `tools/taskrail/src/taskrail/skills/taskrail-autopilot/references/gate-review.md` — *Rebase: at
...
- `.claude/skills/taskrail-autopilot/…` and `.taskrail/installed.json` — through
  `taskrail upgrade`.
- `tools/taskrail/DESIGN.md` §7 CLI table row for `checks`, §7.5, and the last sentence of §12.7's
  `[[autopilot.resource]]` bullet (D4).
- `tools/taskrail/README.md` — none unless D4 is extended; `tools/taskrail/CHANGELOG.md` one
