...
- `tools/taskrail/src/taskrail/prior.py` — new module: artifact, branch and commit signals.
- `tools/taskrail/src/taskrail/cli.py` — `cmd_show` adds `prior_work` to its JSON and text
  output (a small hunk after `kind_descriptor`; `task_dict` in `query.py` is not changed).
- `tools/taskrail/src/taskrail/skills/taskrail/SKILL.md` — step 2; installed copies under
  `.claude/skills/` refreshed.
- `tools/taskrail/tests/test_prior.py` — new tests against temporary git repositories.
- `tools/taskrail/DESIGN.md` (§7 `show` row), `README.md` (`show` line), `CHANGELOG.md`
  (`## Unreleased`).
