...
- A lane runs a check as the stage's `checks` map gives it (`uv run --directory tools/taskrail
...
- `tools/taskrail/src/taskrail/checks.py` (new) — choosing the checks, finding the worktree and
  the lane's resources, running them.
- `tools/taskrail/src/taskrail/cli.py` — `EXIT_CHECK_FAILED = 6`, `cmd_checks`, and the `checks`
  parser registration (a new top-level command; `cmd_claim` is not touched).
- `tools/taskrail/src/taskrail/autopilot/dispatch.py` — none; `ENVIRONMENT_PREFIX` is imported
  from it, not changed.
- `tools/taskrail/src/taskrail/integrations/claude.md` — the two notes (D4).
...
- `tools/taskrail/tests/test_checks.py` (new) for criteria 1–7; `tests/test_autopilot_skill.py`
  (`CORE_CLAUDE_NOTES` and a new test) for criterion 8.
- `tools/taskrail/DESIGN.md` §7 (CLI table row and a *Checks* subsection) and §8 (the `claude`
  integration row) — governing, exact text in D5. Only if approved: the core `taskrail` skill
  (D6).
- `tools/taskrail/README.md` (one line in its command list), `tools/taskrail/CHANGELOG.md` (one
...
- **D4 — the exact integration note text** (`tools/taskrail/src/taskrail/integrations/claude.md`,
