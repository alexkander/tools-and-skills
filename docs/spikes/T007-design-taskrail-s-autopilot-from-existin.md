...
- A coverage matrix from the reference behaviour, the four first-run decision records, the
  lessons of that run, `tools/taskrail/DESIGN.md` and the CLI sources at `ee38f5b`.
...
{ "version": "taskrail-v0.2.0", "integrations": [], "extras": {}, "files": { ".taskrail/bin/taskrail": "3d28…" } }
```
...
- **`merge-tree` against the current mainline** fails for #8–#10 once later merges touched the
  ...
  `docs/features/README.md` and `tools/taskrail/CHANGELOG.md`. Against the mainline it was
...
```bash
REPO=/path/to/tools-and-skills               # a checkout of this branch
TR="uv run --quiet --project $REPO/tools/taskrail taskrail"
...
M=$(mktemp -d) && git clone -q --no-checkout git@github.com:alexkander/tools-and-skills.git $M/repo && cd $M/repo
