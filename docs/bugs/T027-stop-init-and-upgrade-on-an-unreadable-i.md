...
(`uv run --directory tools/taskrail taskrail --root <repo> …`):
...
`read_manifest` in `tools/taskrail/src/taskrail/install.py` folds "the file does not exist"
...
- `tools/taskrail/src/taskrail/install.py`: `read_manifest`, and through it `Installer.__init__`
...
3. A regression test in `tools/taskrail/tests/test_install.py` that installs, replaces the
