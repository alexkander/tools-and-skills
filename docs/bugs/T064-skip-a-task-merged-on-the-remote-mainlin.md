...
`uv run --directory tools/taskrail python repro.py`. For `done` and then for `discard`, it builds a
...
```text
$ uv run --directory /…/.worktrees/T064-skip-a-task-merged-on-the-remote-mainlin/tools/taskrail python /…/T064/repro.py
===== closing with `done` =====
