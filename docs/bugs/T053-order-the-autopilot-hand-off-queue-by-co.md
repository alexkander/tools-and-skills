...
branch (`uv run --project tools/taskrail taskrail --root <repo> …`). Commit times are pinned with
...
set -eu
SRC=<this worktree>/tools/taskrail
BASE=$(mktemp -d)
REPO=$BASE/repo
...
printf '%s\n' 'version = "taskrail-v0.1.0"' '' '[[backlog]]' 'name = "main"' 'prefix = "T"' 'file = "TODO.md"' '' '[columns]' 'custom = []' '' '[autopilot]' 'enabled = true' > "$REPO/.taskrail/config.toml"
...
`_handoff` in `tools/taskrail/src/taskrail/autopilot/status.py` sorts the waiting tasks by the
...
- `tools/taskrail/src/taskrail/autopilot/status.py` — `_handoff` (the only reader of `tip_time`).
- `tools/taskrail/DESIGN.md` §12.1, the `autopilot status` row of the command table: "`queue` (dependencies first, then
  by the branch tip's commit time)" specifies the bug. Governing path: changed only on approval.
- `tools/taskrail/CHANGELOG.md` — an Unreleased entry.
...
Regression test in `tools/taskrail/tests/test_autopilot.py`, next to
