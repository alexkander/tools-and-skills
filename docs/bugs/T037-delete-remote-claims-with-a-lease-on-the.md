...
(base `2312a2a`) as `uv run --directory tools/taskrail taskrail --root <repo> …`.
...
`claims._delete_remote` (`tools/taskrail/src/taskrail/claims.py`, line 157) chooses the lease from
the `force` flag:
...
- `tools/taskrail/src/taskrail/claims.py` — `_delete_remote` (the bug); `rename_branch` and
  `_delete_remote` again for a record without `remote.commit`.
- `tools/taskrail/src/taskrail/cli.py` — `_change_status` (`done`, `discard`): exit 2 after a
